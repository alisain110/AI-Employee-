...
class PlatinumWatchdog:
    def __init__(self, is_cloud=True):
        ...
        self.services = [
            ...
            "dashboard_merger.py",
            "cloud_orchestrator.py",  # Only if cloud
            "email_watcher.py",       # Only if cloud
            "odoo_draft_mcp.py",      # MCP services
            "odoo_execute_mcp.py"     # MCP services
        ]

        # Filter services based on environment
        if not self.is_cloud:
            # Remove cloud-specific services on local
            self.services = [s for s in self.services if "cloud" not in s and "email" not in s]
    ...
    def is_process_running(self, process_name):
        """Check if a process is running using tasklist (Windows) or ps (Linux/Mac)"""
        try:
            if sys.platform.startswith('win'):
                ...
                return process_name in result.stdout and 'grep' not in result.stdout
    ...
    def start_service(self, service_name):
        ...
        try:
            ...
            if sys.platform.startswith('win'):
                ...
                cmd = [sys.executable, service_name]
                ...
                # Unix-like systems - could use nohup or regular start
                cmd = [sys.executable, service_name]
                process = subprocess.Popen(
                    cmd,
                    stdout=subprocess.PIPE,
    ...
    def update_heartbeat(self):
        ...
        try:
            ...
            with open(self.heartbeat_file, 'w') as f:
                ...
                f.write(f"**Data**: {json.dumps(heartbeat_data, indent=2)}\n")
    ...
    def check_and_restart_services(self):
        ...
        for service in self.services:
            if not self.is_process_running(service):
                ...
                # Service is running, reset count for this service
                if service in self.restart_counts:
                    # Keep only recent restart times
                    now = datetime.now()
                    window_start = now - timedelta(seconds=self.time_window)
                    recent_restarts = [
...
def main():
    # Determine if this is cloud or local based on environment variable or argument
    import argparse
